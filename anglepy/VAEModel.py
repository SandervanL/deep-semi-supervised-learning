...
class VAEModel(object):
    ...
    def __init__(self, theano_warning='raise'):
        ...
        allvars = list(v.values()) + list(w.values()) + list(x.values()) + list(z.values()) + [
            A]  # note: '+' concatenates lists
        ...
        self.f_dL_weighted_dw = theanofunction(allvars + [weights], [logpx + logpz - logqz, weights * (
    ...
    def gw_to_numpy(self, gv, gw):
        return gv, gw

    # A = np.ones((1, n_batch))
    def get_A(self, x):
        return np.ones((1, next(iter(x.values())).shape[1]))
    ...
    def dL_dw(self, v, w, x, z):
        ...
        logpx, logpz, logqz, gv, gw = r[0], r[1], r[2], dict(zip(v.keys(), r[3:3 + len(v)])), dict(
    ...
    def dL_weighted_dw(self, v, w, x, z, weights):
        ...
        L_unweighted, L_weighted, gv, gw = r[0], r[1], dict(zip(v.keys(), r[2:2 + len(v)])), dict(
            zip(w.keys(), r[2 + len(v):2 + len(v) + len(w)]))
        self.checknan(v, w, gv, gw)
        gv, gw = self.gw_to_numpy(gv, gw)
