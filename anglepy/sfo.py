...
class SFO(object):
    ...
    def optimize(self, num_passes=10, num_steps=None):
        ...
        for i in range(num_steps):
            ...
            if self.display > 1:
                print("active {0}/{1}, sfo time {2} s, func time {3} s, f {4}, <f> {5}".format(sum(self.active),
                                                                                               self.active.shape[0],
                                                                                               self.time_pass - self.time_func,
                                                                                               self.time_func,
                                                                                               self.hist_f_flat[-1],
                                                                                               mean(self.hist_f[
                                                                                                        self.eval_count > 0, 0])))
        ...
        for i in random.permutation(range(self.N)):
            ...
            for j in random.permutation(range(self.M)):
                ...
                if not isfinite(dfl_err[j]):
                    ...
                    print("           "),
                print("  gradient subfunction {0}, dimension {1}, analytic {2}, finite diff {3}, error {4}".format(i, j,
                                                                                                                   dfl[
                                                                                                                       j],
                                                                                                                   dfl_obs[
                                                                                                                       j],
    ...
    def get_predicted_subf(self, indx, theta_proj):
        ...
        f_pred = self.hist_f[indx, 0] + dot(self.last_df[:, [indx]].T, dtheta)[0, 0] + 0.5 * dot(dtheta.T, Hdtheta)[
            0, 0]
        return f_pred
    ...
    def update_history(self, indx, theta_proj, f, df_proj, skip_delta=False):
        ...
        if self.eval_count[indx] > 1 and not skip_delta:
            ...
            if lddt < self.eps:
                ...
                if self.display > 2:
                    print("Inner product between dgradient and dtheta too small ({0}).  Not adding to history.".format(
                        corr_ddf_ddt))
            else:
                if self.display > 3:
    ...
    def handle_step_failure(self, f, df_proj, indx):
        ...
        if not step_failure:
            ...
            if f < f_lastpos:
                ...
                if self.display > 2:
                    print("step failed, but last position was even worse ( f {0}, std f {1}),".format(f_lastpos, std(
                        self.hist_f[self.eval_count > 0, 0]))),
            else:
                ...
                if self.display > 2:
                    print(
                        "step failed, proposed f {0}, std f {1},".format(f, std(self.hist_f[self.eval_count > 0, 0]))),
                f = f_lastpos
