...
def main(n_z, n_hidden, dataset, seed, gfx=True, _size=None):
    ...
    if dataset == 'mnist':
        ...
        if True:
            ...
            # Doesn't work
            f_enc, f_dec, pp_params = pp.normalize_noise(_x['x'][:, :50000], noise_sd=0.01, global_sd=True,
                                                         toFloat=True)
        else:
            # Doesn't work
    ...
    model = GPUVAE_YZ_X(updates, n_x, n_y, n_hidden_q, n_z, n_hidden_p[::-1], 'softplus', 'softplus', type_px=type_px,
