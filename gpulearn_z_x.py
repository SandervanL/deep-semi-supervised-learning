...
def main(n_z, n_hidden, dataset, seed, comment, gfx=True):
    ...
    if dataset == 'mnist_binarized':
        ...
        size = 32
        train_x, train_y, test_x, test_y = svhn.load_numpy(False,
                                                           binarize_y=True)  # norb.load_resized(size, binarize_y=True)
    ...
    model = GPUVAE_Z_X(updates, n_x, n_hidden, n_z, n_hidden[::-1], nonlinear, nonlinear, type_px, type_qz=type_qz,
                       type_pz=type_pz, prior_sd=100, init_sd=1e-3)
    ...
    def hook(epoch, t, ll):
        ...
        if gfx and epoch % gfx_freq == 0:
            ...
            if 'pca' not in dataset and 'random' not in dataset and 'normalized' not in dataset:
                ...
                if 'out_unif' in w:
                    image = paramgraphics.mat_to_img(f_dec(w['out_unif'].reshape((-1, 1))), dim_input, True,
                                                     colorImg=colorImg)
...
def get_adam_optimizer(learning_rate=np.float32(0.001), decay1=np.float32(0.1), decay2=np.float32(0.001),
                       weight_decay=np.float32(0.0)):
