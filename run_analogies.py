...
if dataset == 'svhn':
    ...
    size = 32
    train_x, train_y, test_x, test_y = svhn.load_numpy(True,
    ...
    if True:
        ...
        model = GPUVAE_YZ_X(None, n_x, n_y, n_hidden, n_z, n_hidden[::-1], nonlinear, nonlinear, type_px,
                            type_qz=type_qz, type_pz=type_pz, prior_sd=100, init_sd=1e-2)
...
if True:
    ...
    if dataset == 'mnist':
        ...
        idxs = np.asarray([[2439, 820, 6590, 24106, 23978, 18466, 191, 20638, 8496, 8779, 25783, 3926, 91, 6904, 2865,
                            9107, 23066, 14359, 24415, 1754]]).T
...
for sample in range(idxs.shape[1]):
    ...
    for i in range(n_y):
        if draw_rows:
            ...
            y[i, (n_batch_w * i):(n_batch_w * (i + 1))] = 1
